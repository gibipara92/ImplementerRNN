...
class Hypernet(nn.Module):
    ...
    def implement_W(self, p, redundant_train=False):
        ...
        # We output 40 * 45 = 1800weights for our two hidden layer network
    ...
    def forward(self, p, input_data):
        ...
        for i in range(dataset_size):
            output = F.elu(F.linear(torch.Tensor(torch.Tensor(input_data[i]).view(-1, 1)), weight=torch.Tensor(weights[i][:, 0].cpu()).view(-1,1)) + weights[i][:, 1].cpu().repeat(num_points, 1))
            output = F.elu(F.linear(torch.Tensor(output), weight=torch.Tensor(weights[i][:, 2:42].cpu())) + weights[i][:, 42].cpu().repeat(num_points, 1))
            output = F.linear(torch.Tensor(output), weight=torch.Tensor(weights[i][:, 43].cpu()).view(1, -1)) + weights[i][:, 44][0].cpu().repeat(num_points, 1)
...
def train_epoch(H, programs, optimizers, epoch, train_H):
    ...
    init_inputs = np.zeros([dataset_size, num_points])
    target = np.zeros([dataset_size, num_points])
    ...
    if not args.no_train_H:
        for ei, i in enumerate(batch):
            init_inputs[ei] = sorted(np.random.uniform(-5.0, 5.0, [num_points]))
            # Compute targets using phase and amplitude
            target[ei] = dataset[2][i] * np.sin(init_inputs[ei]-dataset[3][i])
    if not args.no_train_p and args.no_train_H:
        for ei, i in enumerate(batch):
            init_inputs[ei] = sorted(np.random.uniform(0.0, 5.0, [num_points]))
            # Compute targets using phase and amplitude
            target[ei] = dataset[2][i] * np.sin(init_inputs[ei] - dataset[3][i])
    else:
        for ei, i in enumerate(batch):
    ...
    output = H.forward(torch.stack([programs[i] for i in batch]), init_inputs).view(-1, num_points)
    mse_loss = F.mse_loss(output, target)
    loss = mse_loss# + reg_loss
    loss.backward()
    # Plot output and ground truth
    if epoch % 200 == 0:# and epoch != 0:
        plt.plot(np.array(init_inputs[25]), np.array(target[25]))
        plt.plot(np.array(init_inputs[25]), np.array(output.cpu().detach().numpy()[25]))
        plt.show()
...
def train_net(H, programs, optimizers, epochs, train_H):
    ...
    try:
        for e in range(epochs):
            ...
            if e % 200 == 0 and not args.no_train_H and e != 0:
...
H = torch.load("/home/ubuntu/implementer_data/sine_model.pyt")
...
#programs = torch.load("/home/ubuntu/implementer_data/sine_train_programs.pyt")
#dataset = torch.load("/home/ubuntu/implementer_data/generated_dataset.pyt")
# Run main function
...
dataset[2] = np.linspace(0.1,5)
dataset[3] = np.linspace(0, 0)
...
train_net(H, programs, optimizers, 50, True)
...
args.no_train_p = True

num_points = 1000

train_net(H, programs, optimizers, 2, True)
