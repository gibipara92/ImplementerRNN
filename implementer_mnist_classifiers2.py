...
class Hypernet(nn.Module):
    def __init__(self, p_dim, input_dim, output_dim, dataloader=None):
        ...
        self.fc1 = nn.Parameter(torch.randn(self.lstm_size, 16) * sigma)
    ...
    def implement_W(self, p, epoch, redundant_train=False):
        ...
        im = h2.view(16, 16)
        if epoch % 20 == 0:
            writer.add_image('data/weights', torch.stack([im, im, im]), epoch)
        return h2.view(-1, 256)


    def forward(self, p, input_data, epoch):
        fc_layer = self.implement_W(p, epoch)
        # output = F.conv2d(input=input_data, weight=conv_mat, bias=None, padding=3)
        result = []
        if epoch % 20 == 0:
            im = input_data[0].view(16,16)
            writer.add_image('data/input', torch.stack([im, im, im]), epoch)
        for i in input_data:
            result.append(torch.dot(fc_layer[0], input_data[0].cuda()))
            #output = F.linear(input=input_data.cuda(), weight=fc_layer)
        # return conv_mat
        return torch.sigmoid(torch.stack(result))
        # return weight
...
def train_epoch(H, programs, optimizers, add_noise, epoch, dataloader, train_H, signature=False, task_id=''):
    ...
    batch = []
    idx = []
    for task in train_tasks[:1]:
        truths.append(np.zeros(1))
        truths.append(np.ones(1))
        (idx1, batch1) = next(iter(class_dataloaders[task[0]]))
        (idx2, batch2) = next(iter(class_dataloaders[task[1]]))
        batch.append(batch1)
        idx.append(idx1)
        batch.append(batch2)
        idx.append(idx2)
    ...
    output = H.forward(programs[0], batch.view(-1,256), epoch)
    loss = F.cross_entropy(torch.stack([output, 1-output]).transpose(0, 1), torch.Tensor(truths).long().cuda())
    #print(loss)
    loss.backward()
    #ETA = .000
    #grad_clip = 5.0
...
#dataset_zip = np.load('/home/ubuntu/Downloads/dsprites_ndarray_co1sh3sc6or40x32y32_64x64.npz')
...
# mnist_classes = []
# dataloader = torch.utils.data.DataLoader(dataset, batch_size=64, drop_last=False, shuffle=False, num_workers=int(2))
# for i in range(10):
#      curr_class = []
...
#          for ei, j in enumerate(mnist_target):
#              if i == j:
#                  curr_class.append(mnist_data[ei])
#      mnist_classes.append(torch.stack(curr_class))
# torch.save(mnist_classes, "mnist_classes.pyt")
mnist_classes = torch.load("mnist_classes.pyt")
class_dataloaders = []
for i in range(10):
    c_dataset = utils.TensorDataset(torch.Tensor(range(len(mnist_classes[i]))), torch.stack([torch.Tensor(i) for i in mnist_classes[i]]))
    c_dataloader = utils.DataLoader(c_dataset, batch_size=1, shuffle=True, drop_last=True)
    class_dataloaders.append(c_dataloader)

# function_dataset = utils.TensorDataset(torch.Tensor(range(1024)), torch.stack([torch.Tensor(i) for i in imgs[:1024]]))
...
H = Hypernet(p_dim=args.p_dim, input_dim=args.imsize * args.imsize, output_dim=256, dataloader=class_dataloaders).to(device)
...
train_net(H, programs, optimizers, args.epochs, class_dataloaders, True)


#ani = animation.ArtistAnimation(fig, ims, interval=25, blit=True, repeat_delay=0)
ani = animation.ArtistAnimation(fig, ims2, interval=25, blit=True, repeat_delay=0)
#plt.title("Programs over Time")
ani.save('Groups_over_time.mp4')
...
rand_mnist_ints = random.sample(range(64), 20)
rand_function_ints = random.sample(range(64), 20)
...
#H = torch.load("/home/ubuntu/implementer_data/model15000_20.pyt")
...
plt.title("Programs with 50 initializations")
plt.show()


plt.figure()
plt.title("Target, Best, Losses for programs in Test Set")
for ei, i in enumerate(test_idx):
    plt.subplot(3,14,ei+1)
    # Truth
    plt.imshow(F.conv2d(input=mnist_data[rand_mnist_ints[ei]].view(1, 1, 16, 16), weight=torch.Tensor(generate_translations_dataset()[2][ei]).view(1, 1, 7, 7).cuda(), bias=None, padding=3).view(16,16))
    min_loss = 1000000000
    min_arg = -1
    losses = []
    for j in range(50):
        output = H.forward([programs[14 * j + ei]], mnist_data[rand_mnist_ints[ei]].view(1, 1, 16, 16))
        target = F.conv2d(input=mnist_data[rand_mnist_ints[ei]].view(1, 1, 16, 16), weight=torch.Tensor(generate_translations_dataset()[2][ei]).view(1, 1, 7, 7).cuda(), bias=None, padding=3)
    ...
    #best
    plt.imshow(np.array(H.forward([programs[14 * min_arg + ei]], mnist_data[rand_mnist_ints[ei]].view(1, 1, 16, 16)).cpu().detach().numpy()).reshape(16, 16))
