...
class Hypernet(nn.Module):
    ...
    def forward(self, p, input_data):
        fc_layer = self.implement_W(torch.stack(p))
...
def train_epoch(H, programs, optimizers, add_noise, epoch, dataloader, train_H, signature=False, task_id=''):
    ...
    batch = []
    idx = []
    for task in train_tasks[:64]:
        truth = random.choice([0,1])
        if truth:
            truths.append(np.ones(32))
        else:
            truths.append(np.zeros(32))
        (idx1, batch1) = next(iter(class_dataloaders[task[truth]]))
        batch.append(batch1)
        idx.append(idx1)
    ...
    output = H.forward([programs[math.floor(i / 32)] for i in range(len(idxs))], batch.view(-1,256))
...
for i in range(10):
    ...
    c_dataloader = utils.DataLoader(c_dataset, batch_size=32, shuffle=True, drop_last=True)
