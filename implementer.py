        ...
        for j in range(7):
            for inv in range(2):
                for blur in [0,1,2,3]:
                    ...
                    if inv == 1:
                        filter *= -1
                    features.append(torch.from_numpy(np.array(filter)).view(1, 1, 7, 7).float().cuda())
    return features
...
class Hypernet(nn.Module):
    def __init__(self, p_dim, input_dim, output_dim, dataloader=None):
        ...
        self.lstm2 = torch.nn.LSTMCell(input_size=1, hidden_size=self.lstm_size)
    ...
    def implement_W(self, p):
        ...
        c1 = 0
        c2 = 0
        h1 = h_t1.view(1, self.lstm_size, 1)
        ...
        return h2.view(1, 1, 7, 7), None


    def forward(self, p, input_data):
        conv_mat = self.implement_W(p)
        output = F.conv2d(input=input_data, weight=conv_mat[0], bias=None, padding=3)
...
def train_epoch(H, programs, optimizers, epoch, dataloader, train_H, signature=False, task_id=''):
    ...
    for batch_idx, (data, _) in enumerate(dataloader):
        for program_id, p in enumerate(programs):
            target = F.conv2d(input=data.cuda(), weight=functions[program_id].view(1, 1, 7, 7).float(), bias=None, padding=3)
            if args.cuda:
                data, target = data.cuda(), target.cuda()
            data, target = Variable(data), Variable(target)
            H.optimizer.zero_grad()
            optimizers[program_id].zero_grad()
            output = H.forward(p, data.view(-1, 1, args.imsize, args.imsize))
